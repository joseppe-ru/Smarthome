...
impl HttpWebServer {
    ...
    pub fn new(adr:SocketAddr)->Self{
        HttpWebServer {
            socket_adr: adr,
        }
    }
    async fn handle_websocket_message(&self,message:Message, _tx: &mut SplitSink<WebSocket,Message>){
    ...
    async fn handle_client(&self,web_socket: WebSocket){
        ...
        while let Some(body) = rx.next().await{
            ...
            self.handle_websocket_message(message, &mut tx).await;
        }
        println!("WebSocket verbindung unterbrochen");
    }

    pub async fn http_server_setup(&'static self,shut_channel_rx: oneshot::Receiver<()>) -> Result<(), &'static str> {
        ...
        let (_, server) = warp::serve(routes)
            ...
            //.cert_path("cert.pem")
            //.key_path("key.rsa")
            .bind_with_graceful_shutdown(self.socket_adr, async { shut_channel_rx.await.ok(); });
